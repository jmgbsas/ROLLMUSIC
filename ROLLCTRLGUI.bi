...
Static Shared As HMENU hMessages,MenName1,MenName2,MenName3,MenName4,MenName5,MenName6,MenName7,MenName8,MenName9,MenName10
...
If instancia < ARG3_TITU And ubirtk=0 And ubiroll=0 And menuabierto=0 Then ' rollmusic CON control
     ...
     If (BatchGraficoOCtrl=3 Or BatchGraficoOCtrl=4 Or BatchGraficoOCtrl=5 ) And ubiejec=0 Then
          ...
          hwndC = OpenWindow("RollMusic Ctrl V "+ nroversion,10,10,ANCHO/6,ALTO/12, _
          ...
          Var IMGC=Load_image(ROLLDIR+"recur\Cargar.bmp")
          GroupGadget( GRUPO_BTNS_MANUAL,55,0,113,40,"") 'play cancion
          GadgetToolTip(GRUPO_BTNS_MANUAL,"Cargar, Detener , Ejecutar  ")
          
          ButtonImageGadget(BTN_ROLL_CARGAR, 60,12,25,25,IMGC, FB_BS_PUSHLIKE or BS_BITMAP  )
          ...
          GadgetToolTip(BTN_ROLL_PARAR,"DETENER ejecucion Roll ")
          
          ButtonImageGadget(BTN_ROLL_EJECUTAR, 140,12,25,25,IMGE, FB_BS_PUSHLIKE or BS_BITMAP  )
          GadgetToolTip(BTN_ROLL_EJECUTAR,"EJECUTAR Roll ")
     ...
     GroupGadget(GRUPO_BTNS_MIDI,445,0,153,40,"")
     ...
     ButtonImageGadget(BTN_MIDI_EJECUTAR,490,12,25,25,IMGE, FB_BS_PUSHLIKE or BS_BITMAP  )
     GadgetToolTip(BTN_MIDI_EJECUTAR,"EJECUTAR Grabacion midi")
     
     ButtonImageGadget(BTN_MIDI_GRABAR,530,12,25,25,IMGG, FB_BS_PUSHLIKE or BS_BITMAP  )
     GadgetToolTip(BTN_MIDI_GRABAR,"GRABAR ejecucion midi")
     
     ButtonImageGadget(BTN_MIDI_CARGAR, 570,12,25,25,IMGC, FB_BS_PUSHLIKE or BS_BITMAP  )
     GadgetToolTip(BTN_MIDI_CARGAR,"Cargar un Archivo Ejec")
     
     
     ''''''''''''''''''''''''''''''''''TEXTOS
     TextGadget(TEXT_TOPE,620,12,95,20,"         ")
     StringGadget(LINEA_COMANDO,720,12,300,20,"ENTRE UN COMANDO PULSANDO INICIO ",ES_UPPERCASE,ES_LEFT )
     ButtonGadget(OK,1040,12,60,20,"INICIO")
     '---------------------------------------------------
     ' pistas manuales  PARA CARGAR CANCION DESDE DIRECTORIO PISTAS ECHAS CON ROLL
     GroupGadget( GRUPO_BTNS_MANUAL,95,0,153,40,"") 'play cancion
     ...
     ButtonImageGadget(BTN_ROLL_EJECUTAR, 140,12,25,25,IMGE, FB_BS_PUSHLIKE or BS_BITMAP  )
     GadgetToolTip(BTN_ROLL_EJECUTAR,"EJECUTAR cancion de Tracks, o Grabacion midi Roll ")
     ...
     ButtonImageGadget(BTN_ROLL_CARGAR, 220,12,25,25,IMGC, FB_BS_PUSHLIKE or BS_BITMAP  )
     GadgetToolTip(BTN_ROLL_CARGAR,"Cargar un Archivo Roll (Sin cancion cargada)")
     
     ButtonGadget(BTN_PANIC, 320,8,50,25,"PANIC"  )
     GadgetToolTip(BTN_PANIC,"Boton Panico Resetea Notas y Cierra Puertos Salida")
   ...
   ButtonGadget (BTN_MAS_VOL_EJEC,370,792,20,20,"+")
     ...
     MenName9=MenuTitle (hMessages,"ENTRENAR")
     ...
     Menubar(MenName1)
     menuitem(1005,MenName1,"2.0 Cargar Solo un Roll sin Cancion y Con Grafico")
     ...
     Menubar(MenName1)
     MenuItem(10091,MenName1, "4.6 GRABA UNA PISTA ROLL DIRECTO A MIDI TIPO 0")
     MenuItem(10092,MenName1, "4.7 >Na< A DESARROLLAR *CARGA ARCHIVO MIDI A RTK")
     MenuItem(10093,MenName1, "4.8 >Na< A DESARROLLAR CONVERTIR UN *.RTK CARGADO  A *.EJEC")
     ...
     MenuItem(1017,MenName1, "5.2 RENOMBRAR/BORRAR  UNA SOLA PISTA EJECUCION SELECCIONADA EN (S), (PONER BLANCO LA BORRA)")
     MenuItem(10171,MenName1, "5.2.1 BORRAR <TODAS> LAS PISTAS DE EJECUCION CARGADAS ")
     ...
     Menubar(MenName3)
     MenuItem(1040,MenName3, " 2.0 Cambia Instrumento por orden Alfabetico, ROLL y CANCION")
     ...
     ' =================== TIEMPO Y RITMO ============
     MenuItem(1080,MenName5,"1.0 TEMPO, Por omision=60, Ejecucion Tick por omision=5,2mseg tresillo de semifusa para  Tempo=240")
     ...
     '==============ENTRENAR ================================
     MenuItem(1230,MenName9, "1.0 Escuchar y Cantar Notas puras al Unisono, para entrenar el oido y la Voz")
