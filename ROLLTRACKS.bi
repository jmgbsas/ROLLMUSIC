...
Sub CargarMidiIn(fileMidiIn As String,   ByVal ntkp As Integer, byval version As Integer)
     ...
     If version=2  Then
          ...
          pmEj(ntkp).tipoDelta   = tocaparam2(ntkp).tipoDelta  '' si es 0 es tiempo, si es 1 son nroticks de tickChico 
     Else
...
'''     Dim As Integer maxTotal=tocaparam(ntkp).delta+ tocaparam(ntkp).maxpos 

'''     If  tocaparam2(ntkp).TipoDelta=1 Then
''         ReDim (Toca(ntkp).trk ) (1 To  maxTotal )
''     Else
         ReDim (Toca(ntkp).trk ) (1 To  tocaparam(ntkp).maxpos )
'''     EndIf
     Get #f, ,Toca(ntkp).trk()
'''Dim desplaza As Integer , ind1=0
''If  tocaparam(ntkp).delta > 1 Then  '' ticks de silencio
''    desplaza=tocaparam(ntkp).delta
''    ind1=ind1+1
''     For  p2 As Integer =desplaza To  maxTotal
''            Toca(ntkp).trk(p2).modo= TocaSindelta(ntkp).trk(ind1)  
''            Toca(ntkp).trk(p2).nota  = TocaSindelta(ntkp).trk(ind1)
''            Toca(ntkp).trk(p2).vel    = TocaSindelta(ntkp).trk(ind1)
''     Next  p2   
''EndIf
     Sleep 15
     ReDim Preserve (Toca(ntkp).trk ) (1 To CantTicks)''
     sleep 15
     ...
     Else
          ...
          Do While Len(DirEjecSinBarra) > 0
               ...
               Else
                    fileMidiIn=DirEjecSinBarra
               End If
               If pmEj(nf).versionEJEC=2 Then ' cargar tocaparm2
          ...
          Do While Len(DirEjecSinBarra) > 0 ' If len(filename) is 0, exit the loop: no more filenames are left to be read.
               ...
               cadena = tocaparam(np).nombre 
               cadena= sacarExtension(cadena) 
               cadena= sacarpath(cadena)
...
Sub CargarUnaEjec (lugar As String, ByRef ntkp As Integer, ByRef version As integer)
     ...
     cadena= sacarExtension(cadena) ' BUSCA EL PUNTO NO DEPENDE DE LA EXTENSION
     cadena= sacarpath(cadena)
...
Sub CargarTrack(Track() As sec, ByRef ntk As Integer , ByRef ubirtk As Integer) '', ByRef check As UByte)
     ...
     '     If desde=0 Then
     '       desde=3 ''4
     pmTk(ntk).desde=desde 'kuku AL INICIO DEL PROGRAMA SE AAJUSTA A 3 Y ACA
     '''   messbox("CARGA TRACK DESDE","esta en cero el  track "+Str(ntk))
     '     EndIf
     '     If hasta=0 Then
     '        hasta=8
     pmTk(ntk).hasta=hasta 'kuku AL INICIO DEL PROGRAMA SE AAJUSTA A 8 Y ACA
...
'
Sub RollTempaTrackTemp (TrkTemp() As poli,RollTemp() As datsec,funcion As string)
     On Local Error GoTo falla
     ...
     Print #1,"RollTempaTrackTemp MaxPos ",MaxPos
     ...
     If  Err > 0 Then
          ...
          errmsg = "FAIL Error RollTempaTrackTemp " & Err & _
          " in function " & *Erfn & _
          " on line " & Erl & " " & ProgError(Err)
          Print #1, errmsg
          Print #1, "i1, i2 ", i1,i2
...
Sub ActualizarRollyGrabarPistaTrk (funcion As String, nroejec As Integer)
     ...
     If funcion ="grabartrkcancion" Then
          ...
          Print #1 , "nombre ", titulosTk(ntk)
     End If
     terminar=NO_TERMINAR_CON_DATOS_CARGADOS ''1204-2026
     Parar_De_Dibujar=SI
     ...
     Print #1, " NTK, UBOUND Roll.trk  ", ntk,  UBound(Roll.trk, 1)
     
     If Maxpos > UBound(Roll.trk, 1)  Then
          pmTk(ntk).MaxPos=UBound(Roll.trk, 1)
          MaxPos=pmTk(ntk).MaxPos
          ReDim RollTemp (1 To MaxPos+100, NB To NA) As datsec
     End If
     ...
     Print #1,"posn=k y  MAXPOS ",posn, MAXPOS
     ...
     Else
          ...
          Next i1
     End If
     ' AHORA MAXPOS SE ACHICO o quedo igual
     Print #1,"MaxPos, posn ",MaxPos,posn
     '''' 12-04-2026--MaxPos=posn +6
     ...
     RollTempaTrackTemp (TrkTemp() ,RollTemp(), funcion)
     ...
     ' ----> de TrkTemp a TRack ---> ACA DA ERROR! KOKITO
...
Sub ImportarPistaExterna(nombre As String)
     ...
     If haycorchete> 0 Then
          nom = Mid(nom, haycorchete+1)  ' sacamos path y el [x] , si existe queda nombre con extension
     End If
     ...
     Case ".rtk" ' anda bien seguir probando
          ...
          titulosTk(ntk)=nombre  ' conserva path y numero estan ahi [nn] para ubicarlo en el disco
          CargarTrack(Track(),ntk,1) ' lo carga en ntk nuevo no usa ninguna funcion
          ...
          titulosTk(ntk)=nombre ' cambiamos el nombre de la pista y seubica en el Dir de cancion
          ' grabar en el directorio de cancion
          
          GrabarRollaTrack(0,0,FUNCMENU) ' graba el roll del rtk a directorio cancion
          RecalCompas(ritmo)
...
Sub GrabarRollaTrack ( cambiaext As Integer, nroejec As integer , funcion As string ) ' SE USA PARA TODO
     ...
     ActualizarRollyGrabarPistaTrk (funcion, nroejec) ''''("grabartrkcancion", nroejec)
     FUNCMENU=""
...
Sub RollaTrack (Track() As sec, ntk As Integer,Roll As inst)
     ...
     ReDim (Track(ntk).trk ) (1 To MaxPos+2000,1 To lim3) ''01-07-2026 incremento para borrarzona
     ...
     ' Y ASI UNIFICARIAMOS
     ReDim RollTemp (1 To Maxpos+2000, NB To NA) As datsec
     ...
     ReDim TrkTemp (1 To MaxPos+2000,1 To lim3) As poli 'definimos un Track temporario 01-07-2026 incremento maxpos 
     ' no usamos RollaTrack porque son distintas estructuras RollaTrack usa sec
     ' vector de tracks a solo usamos un track as poli, de modo que dejaremos
     ' un codigo casi duplicado condensado seri amas engorroso creo
