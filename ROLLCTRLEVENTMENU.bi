...
Case 1005 ' cargar un roll desde Control con grafico
     If NombreCancion = ""  Then
          ...
          cadena= sacarExtension(nombre) ' BUSCA EL PUNTO NO DEPENDE DE LA EXTENSION
          cadena= sacarpath(nombre)
          AddListBoxItem(PISTASROLL, cadena) 'SIN EXTENSION
          CerrarGraficodesdeCtrl=0
          Sleep 20
          If (abrirRoll=REABRIR_ROLL_CON_DATOS_CARGADOS And Terminar=NO_TERMINAR_CON_DATOS_CARGADOS )Then
...
''AHORA HAY QUE HACER LO MISMO PERO CARGAIN SERIA EL RTK O UN ROLL , O SEA
'' TOCA RECIBE DE <---- RTK O ROLL -.- 10-07-2026 SEGUIR YA ESTA CLARO COMO HACERLO
''al convertir a ejec desde roll o rtk
''usar el check automatico Rollmusic.bas linea 637
'' If tocatope < 32   Then
...
Dim K1 As Integer
tocatope=tocatope+1
'tocaparam(k).nombre longitud 29 no  contiene el path
' nompista no tiene el path en ejec, y en roll, rtk y Titulos si lo tiene 
'--> reponer y corregir ====>> tocaparam(k).nombre=Titulos(ntk).nombre
   CTRL1209(K1)
...
Case 10166 ''BORRAR UNA EJEC EN PANTALLA 
' SELECCIONARLA Y BORARLA
       CTRL10166 ()     
     '-----------------------------------------------------------------------
Case 1017 'renombrar pista ejecucion y borrado si el nombre es cadena vacia
 ''MUY MOLESTO NO HAY TOOLTIP PARA ITEM MessBox("","SOLO LA GRABACION 1015 GUARDA EL NOMBRE EN EL ARCHIVO")
''NOTA SOLO LA GRABACION COLOCA EL NOMBRE DENTRO DEL ARCHIVO
     mensajeEstado="5.2 Menu Archivo, No Graba el Nombre dentro del Achivo para eso usar además Grabar en 5.0, el nombre guardado se usa en la carga aunque el archivo fisico tenga nombre diferente"   
StatusBarGadget(BARRA_DE_ESTADO, mensajeEstado)
     ...
     ' EL RESTO..
     GrabarEjec=HabilitaGrabar ''permite que se pueda grabar y detener el play
     Dim As Integer nroPista
     ...
     If Len (nompista) = 0 Then
          ...
          CTRL10166() '''veremos que pasa...en los test
          ReordenarContenidoEjecs (nroPista )
          ...
          If tocatope < 0 Then tocatope=0
     End If

Case 10171  'BORRAR TODAS LAS PISTAS DE EJECUCION SELECCIONADAS
''ok anda bien 10-08-2026
     Print #1,"Case 1017 GRABA UNO SOLO O TODOS??? NO ES UN LOOP PORQUE HACE LOOP?"
     Print #1,"**********************************************************************"
     GrabarEjec=HabilitaGrabar 'permite que se pueda grabar y detener el play
     For j3 As Short =1 To tocatope 
       CheckBox_SetCheck( cbxejec(J3),1)
     Next j3
     Sleep 1  
     For J3 As Short = tocatope To 1 Step -1 
        If  CheckBox_GetCheck( cbxejec(Ix))= 1 Then 
              '''''nroPista=GetItemListBox(PISTASEJECUCIONES) +1 ' DEVUELVE A PARTIR DE CERO
          '''Print #1,"'borrar pista y comprimir lista de ejecs si quedo un hueco.."
          DeleteListBoxItem(PISTASEJECUCIONES, J3-1)
            CheckBox_SetCheck( cbxejec(J3),0)
            ''''   ReordenarContenidoEjecs (J3 )''' las no borradas mueve los datos a los lugares vacios
            tocatope=tocatope-1
            ntkp=ntkp -1
            TopeEjec=TopeEjec-1
          CTRL10166() '''borra contenido de la seleccioada
         EndIf
     Next J3

'''''     CTRL10166() '''veremos que pasa...en los test
      If tocatope < 0 Then tocatope=0   End If
      If ntkp       < 0 Then ntkp=0        End If
      If TopeEjec < 0 Then TopeEjec=0  EndIf
      If ntoca      < 0 Then ntoca=0      End If
 '-----------------------------------------------------------------------
...
Case 1060 ' <========== crea track y reemplaza al existente en la edicion
     CerrarGraficodesdeCtrl=0
     Sleep 20
Print #1,"CASE 1060 abrirRoll,  Terminar "; abrirRoll, Terminar
FILEFLUSH(-1)
...
Case 10882 ' Ajuste de cantidad de pulsos sin sonido
   ...
   tic=0
     ...
     '-----------------------------------------------------------------------
Case 10883 ' Reset de cantidad de pulsos y parada del metronomo
     CPCS=0:CPSS=0:GUARDOCPCS=0:GUARDOCPSS=0:cadenapulsos=""
   terminar_metronomo=1
   ''Sleep 100 ' para que tenga efecto la detencion
   '''terminar_metronomo=0 ' habilitamos metronomo , funcionara?, si porque el metronomo esotro thread
   medio_metronomo_on=FALSE
   tic=0
   volalto=1
   MOV_FLAG=0
     '------------------------------------------------------------
Case 10884 ' PULSOS EN UNA CADENA DE  ENTRADA
     
     thread3= ThreadCall CTRL10884 (cadenapulsos)
     tic=0 ' habilito boton metronomo por las dudas si quedo deshabilitado
